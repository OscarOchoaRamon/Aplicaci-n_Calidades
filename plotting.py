...
def create_chart(df, parameter, selected_columns=None, date_angle=-90, legend_position="right", symbol_style="circle"):
    ...
    legend_layout = dict(
        font=dict(family="Bookman Old Style", size=8, color="black"),
        title=dict(text="")
    )
    ...
    fig.update_layout(
        ...
        yaxis_title=dict(text=f"{parameter} ({unit})", font=dict(family="Bookman Old Style", weight="bold", size=9, color="black")),
        
        # Legend logic
        legend=legend_layout,
        
        # Colors & Fonts
        font=dict(family="Bookman Old Style", color="black", size=9),
        paper_bgcolor="white",
        plot_bgcolor="white",
    ...
    if not df['fecha'].empty:
        ...
        fig.update_xaxes(
            ...
            tickvals=tick_vals,
            ticktext=tick_text,
            tickfont=dict(family="Bookman Old Style, Times New Roman, serif", size=9, color="black"),
            tickangle=date_angle, # Configurable angle
            showline=True, linewidth=1, linecolor='black', mirror=True
        )
    else:
        # Fallback if no dates
         fig.update_xaxes(tickfont=dict(family="Bookman Old Style, Times New Roman, serif", size=9, color="black"))
    ...
    fig.update_layout(font=dict(family="Bookman Old Style, Times New Roman, serif", size=9, color="black"))
